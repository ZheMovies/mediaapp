[18:35, 27/02/2025] Wallace: document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchMediaDetails(id, type) {
        try {
            ...
            if (data.errors) {
                console.error('Erro ao buscar d…
[18:46, 27/02/2025] Wallace: document.addEventListener("DOMContentLoaded", () => {
    const urlParams = new URLSearchParams(window.location.search);
    const id = urlParams.get("id");
    const type = urlParams.get("type");
